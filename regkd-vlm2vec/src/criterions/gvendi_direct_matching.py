...
class GVendiDirectMatchingCriterion(nn.Module):
    ...
    def forward(self, distiller, input_data: Dict) -> Dict[str, torch.Tensor]:
        ...
        gs_aligned = F.normalize(gs_aligned, dim=-1)
        gt_projected = F.normalize(gt_projected.detach(), dim=-1)

        L_direct_grad = F.mse_loss(gs_aligned, gt_projected, reduction="sum")
        L_gvendi = cfg.lam_commit * L_direct_grad
