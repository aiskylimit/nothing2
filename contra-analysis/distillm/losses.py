...
def frfd_distillation_loss(
    ...
    if attention_mask.sum() > 0:
        for j in range(num_distill_layers):
            ...
            h_T_current = teacher_hiddens[teacher_schedule[j]].to(device=f"cuda:{device}", dtype=torch.float32)
            # h_S_next = student_hiddens[student_schedule[j+1]].to(device=f"cuda:{device}", dtype=torch.float32)
            
            actual_y_next = projector(h_S_current) if projector is not None else h_S_current
            with torch.no_grad():
                y_S_j = projector(h_S_current.detach()) if projector is not None else h_S_current.detach()
                y_T_j = h_T_current.detach()

                # Curriculum target: h_S + k(h_T - h_S), where k is cur_t.
                target_y_next = y_S_j + cur_t * (y_T_j - y_S_j)
