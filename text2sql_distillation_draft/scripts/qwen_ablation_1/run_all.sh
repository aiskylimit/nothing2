...
INFER_OUTPUT_ROOT="${INFER_OUTPUT_ROOT:-results/infer/qwen_ablation_1}"
...
script_for_variant() {
  case "$1" in
    no_con1)
      echo "scripts/qwen_ablation_1/train_no_con1.sh"
      ;;
    no_con2)
      echo "scripts/qwen_ablation_1/train_no_con2.sh"
      ;;
    no_con1_no_con2|no_both)
      echo "scripts/qwen_ablation_1/train_no_con1_no_con2.sh"
      ;;
    *)
      echo "Unknown ablation variant: $1" >&2
      echo "Supported variants: no_con1,no_con2,no_con1_no_con2" >&2
      exit 2
      ;;
  esac
}
...
for variant in "${VARIANTS[@]}"; do
  ...
  if [[ "${variant_name}" == "no_both" ]]; then
    variant_name="no_con1_no_con2"
  fi
  ...
  bash running.sh \
    ...
    --skip-finalize \
    --filter "${train_script}" \
    --log-dir "run_logs/qwen_ablation_1/${variant_name}" \
    --infer-after-train \
