#!/usr/bin/env python3
"""Collect SynID keywords-weight grid eval logs into per-seed JSON summaries."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--infer-output-root",
        type=Path,
        default=Path("results/infer/synid_ce_keywords_weight/qwen"),
    )
    parser.add_argument(
        "--eval-output-root",
        type=Path,
        default=Path("results/eval/synid_ce_keywords_weight/qwen"),
    )
    parser.add_argument(
        "--required-benchmarks",
        nargs="+",
