...
class Encoder(object):
    ...
    def encode(self, line):
        ...
        if prompt_token_count > self.args.max_prompt_length:
        ...
        if self.args.split == "train" and t_system_prompt is not None and t_user_prompt is not None:
            ...
            t_prompt_token_count = len(t_prompt_tokens)
            if len(t_prompt_tokens) > self.args.t_max_prompt_length:
                t_prompt_tokens = t_prompt_tokens[:self.args.t_max_prompt_length]

            if response_tokens is not None:
                max_response_len = min(
                    self.args.max_length - len(prompt_tokens),
                    self.args.t_max_length - len(t_prompt_tokens),
                )
        ...
        return line, prompt, prompt_tokens, response_tokens, t_prompt_tokens, t_prompt_token_count, len(line)
...
def load_split_data(args):
    ...
    if args.split == "train":
        teacher_file_path = os.path.join(args.data_dir, "teacher_train.jsonl")
        if os.path.exists(teacher_file_path):
            data = merge_teacher_train_data(data, teacher_file_path)

    return data


def merge_teacher_train_data(student_lines, teacher_file_path):
    with open(teacher_file_path, "r", encoding="utf-8") as f:
        teacher_lines = [line for line in f.readlines() if line.strip()]

    student_lines = [line for line in student_lines if line.strip()]
    if len(student_lines) != len(teacher_lines):
        raise ValueError(
            "Student train and teacher train row counts differ: "
            f"{len(student_lines)} != {len(teacher_lines)}. "
            f"Teacher file: {teacher_file_path}"
        )

    merged_lines = []
    for index, (student_line, teacher_line) in enumerate(zip(student_lines, teacher_lines)):
        student = json.loads(student_line)
        teacher = json.loads(teacher_line)

        missing_teacher_fields = [
            field
            for field in ("t_system_prompt", "t_user_prompt", "response")
            if field not in teacher
        ]
        if missing_teacher_fields:
            raise ValueError(
                f"Missing teacher fields at row {index}: {missing_teacher_fields}. "
                f"Teacher file: {teacher_file_path}"
            )

        if student.get("response") != teacher["response"]:
            raise ValueError(
        ...
        merged = dict(student)
        merged["t_system_prompt"] = teacher["t_system_prompt"]
        merged["t_user_prompt"] = teacher["t_user_prompt"]
        merged_lines.append(json.dumps(merged, ensure_ascii=False) + "\n")

    print(f"Merged privileged teacher prompts from {teacher_file_path}")
    return merged_lines
...
def main():
    ...
    for lid, (
        ...
        response,
        t_prompt,
        t_prompt_token_count,
        bytes_processed,
    ) in enumerate(encoded_docs):
        ...
        if args.split in ["train", "valid"]:
            ...
            if args.only_prompt:
                ...
                if args.split == "train" and t_prompt is not None:
                    ...
                    t_prompt_raw_lens.append(t_prompt_token_count)
    ...
    if len(t_prompt_lens) > 0:
        t_truncated_num = sum(
            raw_len > args.t_max_prompt_length
            for raw_len in t_prompt_raw_lens
            if raw_len is not None
        )
        print(
            "Teacher prompt lengths.",
            "Mean:", np.mean(t_prompt_lens),
            "Max:", np.max(t_prompt_lens),
            "Min:", np.min(t_prompt_lens),
            "Raw max:", np.max(t_prompt_raw_lens),
            "Truncated:", t_truncated_num,
        )
