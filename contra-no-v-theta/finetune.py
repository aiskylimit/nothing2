    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if "contra" in args.type:
                ...
                teacher_states_for_contra = teacher_hidden_states
                ...
                if args.cross_tokenizer_sra:
                    ...
                    teacher_states_for_contra = teacher_pooled_states
                    current_student_schedule = list(range(len(hidden_states)))
                    mask = alignment["span_mask"].float()
                contra_loss = frfd_distillation_loss(
                    hidden_states,
                    teacher_states_for_contra,
                    projector,
                    current_student_schedule, current_teacher_schedule, mask,
                    args.num_distill_layers,
                    cur_t=cur_t,
                    device=device
                )
            ...
            model.backward(loss)
            model.step()
            # grad accum for projector
            if step % args.gradient_accumulation_steps == 0:
                update_velocity_dict["optimizer"].step()
                update_velocity_dict["optimizer"].zero_grad()
                update_velocity_dict["lr_scheduler"].step()
            ...
            if "contra" in args.type \
                and velocity_field is not None \
...
def main():
    ...
    if "contra" in args.type and (velocity_field is not None or projector is not None):
        ...
        optimizer_v = get_optimizer(configs, projector)
