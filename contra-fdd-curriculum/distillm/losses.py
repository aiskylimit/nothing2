...
def get_fdd_loss(
    ...
    # mask = (no_model_batch["label"] != -100).int()
    curriculum_k = min(max(float(curriculum_k), 0.0), 1.0)
    ...
    try:
        ...
        if curriculum_k == 1.0:
            target_hiddens_logits = t_hiddens_logits
        else:
            target_hiddens_logits = s_hiddens_logits.detach() + curriculum_k * (
                t_hiddens_logits - s_hiddens_logits.detach()
            )
        traj_loss = soft_label_distill_loss(s_hiddens_logits, target_hiddens_logits, mask.unsqueeze(0)) / len(teacher_schedule)
        
        s_hiddens_logs = F.log_softmax(s_hiddens_logits, dim=-1)
        t_hiddens_logs = F.log_softmax(target_hiddens_logits, dim=-1)
        ...
        if target_hiddens_logits is not None:
            del target_hiddens_logits
        try:
        ...
        for s_hidden, t_hidden in zip(s_hiddens_selected, t_hiddens_selected):
            ...
            if curriculum_k == 1.0:
                target_hidden_logits = t_hidden_logits
            else:
                target_hidden_logits = s_hidden_logits.detach() + curriculum_k * (
                    t_hidden_logits - s_hidden_logits.detach()
                )
            # traj_loss += forward_kl(s_hidden_logits, t_hidden_logits, no_model_batch)
            traj_loss += soft_label_distill_loss(s_hidden_logits, target_hidden_logits, mask)

            s_hidden_logs = F.log_softmax(s_hidden_logits, dim=-1)
            t_hidden_logs = F.log_softmax(target_hidden_logits, dim=-1)
